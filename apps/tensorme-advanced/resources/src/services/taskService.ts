...
class TaskService {
  ...
  public cleanupListeners(): void {
    if (this.sessionV2Listener) {
      this.sessionV2Listener();
      this.sessionV2Listener = null;
    }
    if (this.sessionQueueV2Listener) {
      this.sessionQueueV2Listener();
      this.sessionQueueV2Listener = null;
    }
    this.isListenerActive = false;
    console.log('[TaskService] Cleaned up existing listeners.');
  }
  ...
  public async processChatListeners(sessionId: string, clientReference?: string, userId?: string, chatId?: string): Promise<string> {
    ...
    const taskPromise = new Promise<string>((resolve, reject) => {
      ...
      (resolver as any).sessionId = sessionId;
  ...
  public async processChatTask(session: FormattedSession, messages: Message[], persona?: string, clientReference?: string, userId?: string, chatId?: string): Promise<string> {
    this.ensureListenersReady();
    const finalPrompt = buildPrompt(messages, 'deepseek-r1', persona);
  ...
  private handleTaskStatusUpdate(logs: any[]): void {
    for (const log of logs) {
      try {
        ...
        console.log("eventName", eventName, "sessionId", sessionId, "taskId", taskId, "timestamp", new Date().toISOString());
...
// Singleton pattern for development to prevent issues with HMR
